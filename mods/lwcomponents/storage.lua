...
local function search_filter (v, terms)
	local name = v.description
	if terms then
		for _, t in ipairs (terms) do
			if (name:lower ():find (t, 1, true)) or v.item:find(t,1,true) then
				return true
			end
		end
...
local function indexer_get_formspec (pos, search, exc)
	...
	for _, v in ipairs (inv_list) do
		if search_filter (v, terms) then
			local vitem = hexencode(v.item)
			local stack = ItemStack (v.item)
			local max_stack = stack:get_stack_max ()
			local exc=math.min(max_stack,exc)
			local vitemrs = ItemStack(v.item) vitemrs:set_count(exc)
			local vitemr = minetest.formspec_escape(vitemrs:to_string())
			local descr_esc = minetest.formspec_escape (v.description)
			local elemna=("item_%.1u_%s"):format(exc,vitem)
			local item =
			string.format ("item_image_button[%0.2f,%0.2f;1.0,1.0;%s;%s;]",
								x,y, vitemr, elemna)
			
			item = item..
			string.format ("tooltip[%s;%s]",
								elemna,
								"("..v.count..") " .. descr_esc)
	...
	local excs={1,10,100}
	local exci={}
	for k,v in pairs(excs) do exci[v]=k end

	local spec =
	string.format ("formspec_version[3]"..
						"size[21.75,14.5,false]"..
						("dropdown[1,1.21;2,0.8;excount;%s;%s;]"):format(table.concat(excs,","),exci[exc] or 0)..
						"field[3.0,1.21;7.5,0.8;search_field;;%s]\n"..
...
local function indexer_on_receive_fields (pos, formname, fields, sender)
	...
	for k, v in pairs (fields) do
		local iqty,item=k:match("^item_([^_]+)_(.+)")
		iqty=tonumber(iqty)
		item=hexdecode(item or "")
		if iqty and item then
			output_items (pos, item, iqty)
			outs=true
		end
	end
	...
	elseif fields.search or (fields.key_enter_field and
									 fields.key_enter_field == "search_field")
		     or (fields.excount and not outs) then
		local meta = minetest.get_meta (pos)

		if meta then
			meta:set_string ("formspec", indexer_get_formspec (pos, fields.search_field,tonumber(fields.excount)))
		end
	end
end
	...
	if utils.pipeworks_supported then
		return
		{
			...
			insert_object = function (pos, node, stack, direction)
				...
				if inv then
					local ret={inv:add_item ("input", stack)}
					store_input (pos)
					return unpack(ret)
				end
